            ...
            keyLoop: for (let pIdx = 0; pIdx < profilesToUse.length; pIdx++) {
              ...
              modelLoop: for (let mIdx = 0; mIdx < cascade.length; mIdx++) {
                ...
                for (let attempt = 0; attempt < totalAttempts; attempt++) {
                  ...
                  try {
                    console.log(`[AI Tailor] [${profile.name}] Attempting model: ${currentModel} (attempt ${attempt + 1}/${totalAttempts})...`);
                    ...
                    sendStatus(`✨ [${profile.name}] Success! Received complete response from ${currentModel} (attempt ${attempt + 1}/${totalAttempts}) in ${attemptDuration}s.`);
